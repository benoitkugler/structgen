...
JSON itfNameToJson(ItfName item) {
  if (item is Concret1) {
    return {'Kind': "concret1", 'Data': concret1ToJson(item)};
  } else if (item is Concret2) {
    return {'Kind': "concret2", 'Data': concret2ToJson(item)};
  } else {
    throw ("unexpected type");
  }
}
...
WithExternalRef withExternalRefFromJson(dynamic json_) {
  final json = (json_ as JSON);
  return WithExternalRef(contextFromJson(json['Field1']),
      contextFromJson(json['Field2']), dictIntRatFromJson(json['Field3']));
}
