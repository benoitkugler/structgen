...
func NewHandler(enumsTable enums.EnumTable) *handler {
	return &handler{enumsTable: enumsTable, interfaces: make(map[string]*union)}
}
...
type handler struct {
	...
	interfaces map[string]*union
	types      []withOrigin // we need to have discovered all types to handle interfaces
}

func (d *handler) HandleType(typ types.Type) loader.Type {
	out := d.analyseType(typ)
	d.types = append(d.types, withOrigin{goType: typ, dartType: out}) // used in processInterfaces()
	return out
}
...
func (d handler) analyseType(typ types.Type) dartType {
	...
	if isNamed {
		...
		if inter, isInter := typ.Underlying().(*types.Interface); isInter {
			...
			out, has := d.interfaces[finalName]
			if !has {
				out = &union{
					origin: origin,
					name_:  finalName,
...
func (h handler) processInterfaces() {
	for _, itf := range h.interfaces {
		for _, t := range h.types {
			if itf.type_ == t.goType.Underlying() {
				continue
			}

			if types.Implements(t.goType, itf.type_) {
				if cl, isClass := t.dartType.(*class); isClass {
					cl.interfaces = append(cl.interfaces, itf.name_)
				}

				itf.members = append(itf.members, t.dartType)
			}
		}
	}
}
