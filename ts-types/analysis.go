...
func (d handler) createType(typ types.Type) Type {
	...
	if isNamed {
		...
		underlyingTsType := d.createType(typ.Underlying())
	...
	case *types.Map:
		return nullableTsType{Type: dict{
			key:  d.createType(typ.Key()),
			elem: d.createType(typ.Elem()),
		}}
	}
	// unhandled type:
	return TsAny
